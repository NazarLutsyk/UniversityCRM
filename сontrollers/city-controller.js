...
controller.create = async function (req, res, next) {
    try {
        if (!ObjectHelper.has(req.body, db.city.requiredFileds)) {
            return next(new ControllerError('Missed required fields! ' + db.city.requiredFileds, 400, 'City controller'));
        }
...
controller.update = async function (req, res, next) {
    try {
        ObjectHelper.clean(req.body, db.city.notUpdatableFields);
